...
export class DiplomacyOverlay extends BaseOverlay {
  ...
  _drawRight(ctx, x, y, w, h) {
    ...
    if (inc.length === 0) {
      ...
      for (const ev of inc.slice(-6).reverse()) {
        ctx.fillStyle = THEME.textSecondary;
    ...
    const btnH = 28;
    const canWar = rel.state !== 'war' && isContact;
    const canPeace = rel.state === 'war' && isContact;

    this._drawActionButton(ctx, x + pad, iy, btnW, btnH, '⚔ WYPOWIEDZ WOJNĘ', canWar, 'danger');
    if (canWar) this._addHit(x + pad, iy, btnW, btnH, 'declare_war', { empireId: this._selectedId });

    this._drawActionButton(ctx, x + pad + btnW + pad, iy, btnW, btnH, '☮ ZAPROPONUJ POKÓJ', canPeace, 'primary');
    if (canPeace) this._addHit(x + pad + btnW + pad, iy, btnW, btnH, 'offer_peace', { empireId: this._selectedId });
  }
