...
// Warstwa ikon PNG dla surowców — obrazy nad emoji z RESOURCE_ICONS.
//
...
// surowca, więc nic się wizualnie nie psuje (zero regresji).
//
...
const ICON_DIR = 'assets/icons/resources';
...
export function loadResourceIcons() {
  ...
  for (const [id, file] of Object.entries(RESOURCE_ICON_FILES)) {
    const path = `${ICON_DIR}/${file}.png`;
  ...
  _loadPromise = Promise.all(promises).then(() => {
    _loaded = true;
    const total = Object.keys(RESOURCE_ICON_FILES).length;
...
export function drawResourceIcon(ctx, id, x, centerY, size = 16, fallbackEmoji = null) {
  ...
  const emoji = fallbackEmoji ?? ALL_RESOURCES[id]?.icon ?? id;
