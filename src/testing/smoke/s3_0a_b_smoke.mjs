...
const { MINED_RESOURCES }      = await import('../../data/ResourcesData.js');
const { ELEMENT_TO_RESOURCE }  = await import('../../data/ElementsData.js');
const { RESOURCE_ICONS }       = await import('../../data/BuildingsData.js');
const { DepositSystem }        = await import('../../systems/DepositSystem.js');
...
header('T5: migracja łańcuch v78→v79→v80');
{
  ...
  assert(m.version === CURRENT_VERSION, `migrated.version === CURRENT_VERSION (got ${m.version})`);
