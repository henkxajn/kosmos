...
> **204**, **205**, **207**, **208**, **210**, **211**; **ZAMKNIĘTE 2026-08-31 w slice'ie
...
| **210** | 🔴 | **Drabina odmów ocenia WYŁĄCZNIE GŁOWĘ rankingu wartości ⇒ AI nie zaatakuje nawet tego, co MOŻE wziąć.** `launchStrike` bierze jeden cel z `pickTarget` i przy `target_beyond_reach` kończy — bez fall-through do pierwszego OSIĄGALNEGO. Sort po `needed` rozstrzyga tylko remisy wartości, a `targetValue` jest per-UKŁAD (**202**) ⇒ wartość cross-system dominuje nad kosztem zdobycia | 🔴 **ZMIERZONE NA ŻYWO** (live-gate §7.3/§7.4): Alioth `needed 1` nietknięte, bo głowa `sys_home` (wartość 227) → `needed 9`. **Lustro 199**: „nie atakuje tego, co może wziąć”. ⚠ Ujawnione dopiero przez commit 3 — wcześniej wszystkie ciała gradowały się identycznie. **Audyt przed naprawą**; sprawdzić, czy domyka się bez **202** |
