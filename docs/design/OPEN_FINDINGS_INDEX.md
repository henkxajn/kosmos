...
| **130-158 · 161-183** | `VESSEL_ORDERS_PLAN.md` §7 + §Findings z live-gate'ów |
...
| **181** | 🟠 | **(c) zdolność** — `quantum_computing` i `military_logistics` poza planem badań OBU archetypów, `fusion_power` poza planem industrialisty; kaskada: `warp_cores` = quantum_cores + **antimatter_cells** ⇒ nieosiągalne u industrialisty mimo `ion_drives`. ⚠ **EXPANSIONIST ma własną `researchQueue`** — diagnoza jest per archetyp | ⚠ **forma słabsza niż tytuł**: zmierzono produkcję `quantum_processors` (18 szt.) MIMO braku techu ⇒ tech-gate `FactorySystem.allocate()` wraca poza trybem `manual`, a AI jest w `reactive`. **Slice (c) nie rusza przed rozstrzygnięciem** |
