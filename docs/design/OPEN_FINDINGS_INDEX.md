...
| **130-158 · 161-184** | `VESSEL_ORDERS_PLAN.md` §7 + §Findings z live-gate'ów |
...
| **181** | 🟠 | **(c) zdolność** — `quantum_computing` i `military_logistics` poza planem badań OBU archetypów, `fusion_power` poza planem industrialisty; kaskada: `warp_cores` = quantum_cores + **antimatter_cells** ⇒ nieosiągalne u industrialisty mimo `ion_drives`. ⚠ **EXPANSIONIST ma własną `researchQueue`** — diagnoza jest per archetyp | ⚠ **ZAKRES ZAWĘŻONY POMIAREM:** hipoteza o omijaniu bramki na ścieżce reaktywnej **WYCOFANA** — gate stoi na wszystkich ścieżkach (`_reactiveAllocate:1265`, `_allocateOrderFP:1158`, `enqueue`, skanery popytu). **Kolonie gracza bezpieczne.** Sprzeczność wyjaśnia **184**: `quantum_processors` przechodzi przez `quantum_physics` ⇒ **wypada z (c)**; zostają `antimatter_cells` (industrialista) + `military_supplies`. `warp_cores` = wyłącznie kaskada |
| **182** | 🟠 | **(a) priorytet** — cel zapasu tier 3+ = **1 szt.**, `safety` jest **5. z 6** w `DEFAULT_REACTIVE_ORDER`, a `demandBonus` AI obejmuje tylko 6 towarów | **ZMIERZONY** (BALANS, wariant tier3+ = 50): `plasma_cores` 0→50, `quantum_cores` 0→3–9, zera 11→9–10; koszt: bez placówki 1/16→2/16, naruszenia 39→41 (szum). Nie rusza `warp_cores`. Gotowy do podpisu |
