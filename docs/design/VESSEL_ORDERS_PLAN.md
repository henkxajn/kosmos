...
154. 🔴 **`AutoRetreatSystem._findNearestFriendlyPlanet` DALEJ nie ma terminu ukladu — i jest ZYWA.**
     ...
     **i ten stan idzie do ZAPISU**. Ta sama klasa co `_freezeAsStationary` w arcu RETREAT_TARGET.
     ...
     ✅ **ZAMKNIĘTY 2026-09-09 — commit `9e1e7d8`, LIVE-GATE PASS 6/6** (slice PRODUCENT-SIDE,
     razem z 263 i legiem Powrotu z 255). Obie żywe ścieżki przeszły na
     `nearestOwnColonyBodyInSystem` (`utils/RetreatTarget.js`, D-FDh):
     `FleetManagerOverlay._handleFleetReturnBase` i `FleetCommandPanel._fleetReturn`. Sierota
     `_findNearestFriendlyPlanet` **USUNIĘTA** (~35 linii) wraz z jej jedynymi konsumentami
     importów (`EntityManager`, `DistanceUtils` w `AutoRetreatSystem`).
     **Lekarstwo jest DROP-INEM — zweryfikowane, nie założone:** identyczna zwrotka
     `{colony, planet, distanceAU}`, ten sam filtr własności, **zachowany fallback placówkowy**
     (placówka jako jedyna w układzie nadal jest wybierana), plus termin układu; statek
     w tranzycie warp ⇒ `null`. ZMIERZONE przed naprawą: statek w `sys_020` dostawał
     `p_home` [sys_home] jako **„2.00 AU"**, przy własnej koloni **6.00 AU** dalej w JEGO
     układzie — obce ciało wygrywało RANKING, nie tylko „bywało wybierane".
     ⚠ **`FleetCommandPanel` był zweryfikowany PO RAZ PIERWSZY** — ta powierzchnia nie miała
     ani pomiaru, ani pinu; okazała się defektywna znak-w-znak jak FMO.
     ⚠ **Brak własnej koloni w układzie ⇒ UCZCIWA ODMOWA**, dokładnie jeden komunikat
     `fleet.noFriendlyPlanet` (EN „No friendly planet in range") — klucz ISTNIAŁ w obu językach,
     **zero nowych kluczy i18n**. Keeper `return_dock_family_smoke` T1/T2/T9/T10.

255. ✅ **ZAMKNIĘTY 2026-09-10 — CAŁA TABELA NÓG DOMKNIĘTA** (Powrót `9e1e7d8` · klik na mapie `92e075f` · pickery flotowe `25363b6`+`9e53840`, live-gate PASS 7/7). Treść pierwotna poniżej, bo to ona nazywa MECHANIZM całej rodziny.
     🔴 **Trzy producenty „Powrotu do bazy" omijaja bramke ukladu, bo podaja GOLY punkt (mechanizm 154).**
     ...
     **Osobny slice, wlasny podpis. NIE laczyc z UI.**
     ...
     🟠 **LEG POWROTU ZAMKNIĘTY 2026-09-09 (`9e1e7d8`, live-gate PASS 6/6) — FINDING ZOSTAJE
     OTWARTY dla legu MAPY.** Rozdział, który okazał się sednem: 255 ma **dwie nogi**, zamykane
     przez różne rzeczy.
     ...
     | **pickery flotowe** (wiersze 8/9) | punkt z pickera w ramce kamery, członkowie floty gdzie indziej | ✅ **ZAMKNIĘTA 2026-09-10** — D-89a..D-89d (niżej), **live-gate PASS 7/7** — `FMO:4732`, `FleetCommandPanel:348` |
     ...
     **DWA site'y, ZERO nowych pól w `spec`, ZERO zmian w MOS**, i reużywa kanału, który już
     istnieje (`RightClickMenu:325-346` nazywa każdy pominięty statek w Dzienniku z powodem).
     Pokrywa wiersze 1/2 — czyli dokładnie repro właściciela. Wiersze 8/9 zostają otwarte
     i **muszą być tak nazwane w gate'cie**, nie przemilczane.

     ⚠ **STEMPEL RAMKI (`spec.frameSystemId`) — ZAPARKOWANY JAKO WŁASNA DECYZJA, nie odrzucony.**
     Zmierzone (audyt 2026-09-09): **działa** — zamyka repro, NIE dotyka POI (fail-open przy braku
     stempla ⇒ Finding 152 bez zmian), NIE wymaga migracji, NIE psuje legalnego gołego punktu
     in-system, a `FleetSystem:137` (`specBase = { ...doctrineMod }`) przenosi go przez fan-out
     floty ZA DARMO. Koszt: **nowe pole w `spec` + 4-6 producentów + udokumentowany wyjątek POI
     + bramka w MOS** — czyli potrojenie powierzchni wobec legu D. `activeSystemId` JEST właściwym
     źródłem ramki (pisze go `StarSystemManager:152`, czyta `ThreeRenderer` w czterech miejscach),
     ...
     **Outliner CTRL+klik** (`Outliner:665`, grupuje statki pod nagłówkami układów, `toggleSelection`
     ...
     ✅ **WIERSZE 8/9 ZAMKNIĘTE 2026-09-10 — decyzje D-89a..D-89d, live-gate PENDING.**
     Dwa pickery flotowe budują spec **własnym literałem obiektu w callbacku**, z pominięciem
     ...
     Łańcuch zmierzony WYKONANIEM, na prawdziwych metodach prototypów (oba pliki importują się
     pod node z `../headless/env.js`): `setPickerMode('targetPoint', cb, {intent, fleetId})` →
     klik w mapę 3D → `GameScene._handleLeftClick:5924` → `_finalizeTargetPointPicker:5934` →
     `cb(point)` → `issueFleetOrder`. Repro `mo_6` odtworzone co do liczb: oba statki z `sys_020`
     dostają `moveToPoint` do (15.71, −158.94) mierzonych od WŁASNEJ gwiazdy.

     🔴 **UI NIE MILCZAŁO — UI KŁAMAŁO.** `_announceFleetOrderResult:4855` / `FCP._announce:337`
     emitują WYŁĄCZNIE `ui:toast` z licznikami, bez nazw, bez powodu i bez wpisu w Dzienniku.
     ZMIERZONE: przy rozkazie wysłanym w cudzą ramkę gracz dostawał **„Flota: 2/2 statków wykonuje
     rozkaz"** — aktywne potwierdzenie sukcesu. ⚠ Ten sam helper ma `if (!res) return`, więc przy
     odmowie NIE POWIE NIC — dlatego głośność odmowy musi być w producencie **jawna** (pinuje F3,
     razem z pinem „zero toastów sukcesu").

     **D-89a — termin przy FINALIZACJI, w OBU callbackach.** ⚠ Bramka ARM-time jest **NIEPEŁNA,
     i to pomiar, nie preferencja**: nic nie kasuje pickera na `system:switched` (dwaj konsumenci
     — `GameScene:3435` i `FMO:505` — go nie dotykają), więc gracz uzbraja picker przy kamerze
     ZGODNEJ z flotą, przełącza układ i klika. ZMIERZONE: picker „NADAL UZBROJONY", oba statki
     z `sys_020` dostają współrzędne z ramki `sys_home` ⇒ bramka ARM-time przepuściłaby dokładnie
     ten przypadek (fałszywy pozytyw) i jednocześnie odmówiłaby graczowi, który chciał NAJPIERW
     przełączyć układ (fałszywy negatyw). Pinuje F7 z kontrolą „picker po przełączeniu uzbrojony".
     ARM-time nie doszedł nawet jako ostrzeżenie: jedyny reużywalny tekst
     (`vessel.reasonTargetOtherSystem`, „najpierw skok warp") mówi o CELU, którego w chwili
     uzbrajania jeszcze nie ma — byłby to nowy klucz i18n, czyli nie „trywialnie".

     **D-89b — predykat ma JEDNO źródło: NEW `src/utils/CameraFrame.js`** (`outOfCameraFrame` /
     `fleetOffendersOutOfFrame` / `describeOrderFail` + `CAMERA_FRAME_REASON`). Trzech konsumentów
     (`RightClickMenu`, oba pickery), zero bliźniaków — `RightClickMenu` straciło trzy prywatne
     metody i osierocony `_pascalCase`. ⚠ To PIERWSZY moduł w `src/utils/` importujący `t()`;
     powód nazwany w nagłówku (jeden formatter „nazwa (powód)" dla trzech powierzchni; alternatywą
     były trzy kopie). „Pure" znaczy tu to, co w kanonie — samodzielny moduł bez cykli, testowalny
     pod node: `RetreatTarget.js` importuje pięć modułów, `StationGroup.js` dwa.

     **D-89c — REGUŁA (obowiązuje poza tym slice'em): termin ramki kamery liczy się przy
     FINALIZACJI, NIGDY przy ARM.** Dziedziczy ją **267** (`patrolManual`); jego mitygacja
     UX-owa (stracone waypointy — np. odmowa przy PIERWSZYM waypoincie zamiast po ENTER) zostaje
     decyzją właściciela w jego własnym slice'ie. ⚠ `patrol` **nie wchodzi** do
     `POINT_SOURCED_ORDER_TYPES` — POI nie ma `systemId` (Finding 152), a T8e to pinuje.

     ⚠ **D-LD2 (cała flota albo nic) NIE jest wyborem, tylko wymuszeniem:** `issueFleetOrder`
     liczy `_arrivalSyncYear` ze WSZYSTKICH eligible (`FleetSystem:150-186`), fan-outuje do
     wszystkich i **nie przyjmuje listy wyjątków** — częściowa wysyłka po cichu złamałaby kontrakt
     zsynchronizowanego przylotu. Odmowa wraca **PRZED** `issueFleetOrder:118`, które kasuje
     poprzedni rozkaz floty ⇒ odrzucony klik nie psuje bieżących rozkazów.

     ⚠ **FINALIZATOR MA DZIŚ JEDNĄ ŻYWĄ POWIERZCHNIĘ — ZMIERZONE.** `_finalizeTargetPointPicker`
     ma dwóch wołających: `GameScene:5934` (klik w mapę 3D) i `GameScene:5201`
     (`ui:targetPointPickerFinalize`). To zdarzenie ma JEDNEGO producenta — `FMO:1653`, mapa 2D
     wewnątrz overlaya, bramkowana `_activeTab==='tactical' && _mapBounds`; `_mapBounds` pisze
     WYŁĄCZNIE `_drawCenter:4946`, które biegnie tylko w gałęzi NIE-rejestrowej (`FMO:785`),
     a `commandTacticalMap:false` + `fleetRegistry:true` wymuszają `_tacticalView='registry'`
     (`FMO:767-769`) ⇒ **`FMO:1653` jest dziś martwe**. Zbiór callbacków trybu `targetPoint`:
     `FMO:4736` (`fleet_move`), `FCP:352` (`fleetcmd_move`), `FCP:364` (`fleetcmd_engage`),
     `GameScene:5260` (`create_poi`, tylko gdy PPM nie podało `worldPoint`), `GameScene:1964`
     (debug). **267 NIE dzieli tego finalizatora** — `patrolWaypoints` idzie
     `addPickerWaypoint` → ENTER → `UIManager.finalizePickerMode()` (`GameScene:4674`).
     ⇒ bramka w finalizatorze dotknęłaby tworzenia POI, ale NIE 267; dlatego termin siedzi
     w callbackach (D-89a), nie w plumbingu.

     ⚠ **D-89d — POPRZEDNI ZAPIS TEJ SEKCJI BYŁ BŁĘDNY: „T9a PADNIE, gdy wiersze 8/9 zostaną
     domknięte" jest NIEPRAWDĄ i zostało to ZMIERZONE.** T9a wołało `issueFleetOrder` **WPROST**,
     czyli seam, który MUSI zostać przepuszczalny — tędy idzie „Powrót do bazy" z celem w ramce
     STATKU (T4/F6). Ta funkcja dostaje GOŁE LICZBY: `{x:220, y:0}` (Powrót) i
     `{x:15.71, y:−158.94}` (klik) są dla niej **nie do odróżnienia**. Sonda na symulowanej
     naprawie: producent odmawia i nazywa winowajców, a stara asercja dalej zwraca
     `{ok:true, accepted:2}` ⇒ pin świeciłby na zielono przez cały slice. **T9a przecelowany na
     PRAWDZIWEGO producenta (execution pin)**, stara asercja została jako **T9a-ctl**: kontrola
     przepuszczalności fan-outu, siostra T4. **T9b (267) zostaje ODWRÓCONY, nietknięty.**
     ⚠ Przy okazji wyszło, że **T5a przechodziło JAŁOWO** po ekstrakcji (degradowało do `null`,
     a `null` jest tym, czego asertuje) — T5a/T7b/T8d przecelowane na żywą ścieżkę.

     **Testy:** NEW `fleet_move_picker_frame_smoke` **42/42**, fail-first **28 PASS / 14 FAIL**
     zmierzony FINALNYMI pinami na `git archive HEAD` (bez dotykania drzewa roboczego);
     `map_click_frame_smoke` **41/41**, fail-first **40/1** (pada dokładnie przecelowany T9a).
     Sweep **223/223 0 FAIL**, `check-i18n` PASS, zapis **v101 bez migracji**, **zero nowych
     kluczy i18n**, bez flagi (rollback = revert — wzór rodziny 147/154/255).
     ⚠ Import keepera `map_click_frame` po `CameraFrame.js` jest **dynamiczny w try/catch**:
     statyczny wywaliłby CAŁĄ suitę na kodzie sprzed naprawy (`ERR_MODULE_NOT_FOUND`) i żaden pin
     nie miałby koloru — to lekcja „pin musi degradować, nie przerywać", tylko na poziomie MODUŁU.

     ✅ **LIVE-GATE 2026-09-10 — PASS 7/7** (właściciel, klient EN, `fleet_1` = `v_13` + `v_18`).
     **§1** wiersz 8 (🎯 Move, overlay zamknięty ✕): odmowa — wpis Dziennika `warn` z OBIEMA nazwami
     i dokładnym tekstem powodu, **BEZ toasta**, `activeOrder:null`, obaj członkowie `NONE/none`.
     **§2** wiersz 9 (`bgMove`, panel pływający): identycznie. **§3** flota mieszana: nazwany WYŁĄCZNIE
     winowajca, **nikt nie ruszył** (D-LD2 na żywo). **§4** 🔑 uzbrojenie przy zgodnej kamerze →
     przełączenie układu W TRAKCIE celowania → klik: **odmowa** — **reguła D-89c zademonstrowana NA ŻYWO**,
     nie tylko keeperem. **§5** kontrola: kamera na układzie floty → rozkaz wydany, toast
     `Fleet: 2/2 vessels executing`, zero wpisów. **§6** Powrót do bazy floty spoza ramki, OBIE
     powierzchnie: działa, leci do koloni WE WŁASNYM układzie — **przepuszczalny seam (F6/T4) na żywo**.
     **§7** leg D, PPM na pojedynczym statku spoza ramki: odmowa bez zmian.

     ⚠ **DWIE NOTATKI Z PRZEPISU GATE’U, obie kosztowały próbę (do powtórzenia przy następnym):**
     (a) czytnikiem flot w konsoli jest **`KOSMOS.fleetSystem.getAllFleets()`** — sięgnięcie po
     `_fleets` zwraca `[]` i wygląda jak „brak flot"; (b) wiersz 8 wymaga zamknięcia overlaya
     **przyciskiem ✕, NIE Esc** — Esc kasuje NAJPIERW picker (`GameScene:4655`), a `FMO.close()`
     pickera nie rusza. Potwierdzone na żywo.

     ✅ **TYM SAMYM CAŁA TABELA NÓG FINDINGU 255 JEST ZAMKNIĘTA** — Powrót (154/`9e1e7d8`),
     klik na mapie (leg D/`92e075f`), pickery flotowe (`25363b6` + `9e53840`). Poza rodziną zostają
     **osobne numery**, nie „resztki 255": **256** (dock — obie połowy + reguła grupowa, decyzja
     właściciela), **267** (`patrolManual` — moment odmowy ROZSTRZYGNIĘTY regułą D-89c, otwarta
     tylko mitygacja UX) i **268** (`engage`, rodzina 166).

     ⚠ **DECYZJA ZAPISANA: ostrzeżenie ARM-time NIE weszło, i to z powodu i18n, nie z lenistwa.**
     Podpis dopuszczał je „jeśli trywialne". Jedyny reużywalny tekst
     (`vessel.reasonTargetOtherSystem` — „najpierw skok warp") mówi o CELU, którego w chwili
...
256. ✅ **ZAMKNIĘTY 2026-09-10 — OBIE POŁOWY** (oferta + admisja), podpis właściciela **„A + C”**. Treść pierwotna poniżej, bo nazywa mechanizm; zamknięcie na końcu wpisu.
     ...
     🔴 **KOREKTA 2026-09-09: „przełączenie jednego argumentu" JEST NIEPRAWDĄ — zmierzone.**
     Leg C tego slice'u miał to zrobić i **WYPADŁ Z ZAKRESU po pomiarze**, nie z wygody:
     1. **`sameSystemOnly: true` BEZ `vessel` jest NO-OPEM.** `filterDockTargets` ma guard
        `if (!vessel) return list`, a **obie** stare powierzchnie wołają
        `openDockPicker(ids, { onDone })` — bez `vessel` (`FleetGroupPanel:443`,
        `FleetCommandPanel:472`). ZMIERZONE: `filterDockTargets(all, null, true)` zwraca
        **wszystkie trzy** ciała (`["p_home","h2","f_far"]`), a z `vessel` — `["f_far"]`.
        Czyli **dwa argumenty na site, nie jeden**.
     2. **⚠ POTRZEBNA DECYZJA WŁAŚCICIELA, nie argument: REGUŁA DLA GRUPY ROZPIĘTEJ NA DWA
        UKŁADY.** Picker jest grupowy, a `vessel` jest jeden. ZMIERZONE: reprezentant `Alfa`
        [sys_home] daje ofertę `[p_home, h2]`, `Beta` [sys_020] daje `[f_far]` — **którykolwiek
        wybór zostawia część statków z celem w obcej ramce**, a `_issueDock` tego NIE broni
        (`{ok:true}` dla `Beta` na `p_home`). Trzy kształty do wyboru:
        **(a)** reprezentant = pierwszy żywy (idiom `_fleetReturn`/`_handleFleetReturnBase`) —
        najmniejsza zmiana, ale dla grupy cross-system nadal częściowo błędna;
        **(b)** PRZECIĘCIE — tylko ciała w układzie KAŻDEGO członka; dla grupy cross-system
        picker idzie pusty i to jest poprawne;
        **(c)** admisja per statek — `_issueDock` dostaje termin układu i odmawia głośno tym
        członkom, którzy nie pasują (to jest druga, ADMISYJNA połowa 256).
     3. ✅ **Komunikat pustego pickera jest DARMOWY** — `BodyPickerModal:51-55` już renderuje
        `t('bodyPicker.empty')`, klucz istnieje PL („Brak dostępnych kolonii") i EN („No colonies
        available"). **Zero nowych kluczy i18n** przy każdym z trzech kształtów.
...
259. ✅ **[ZAMKNIĘTY 2026-09-08, `6b8bb09` — LIVE-GATE PASS 2026-09-08] `Transport` odmawia statkowi w stanie `orbiting` + `idle` — dziura MIĘDZY DWOMA
     ...
     **MECHANIZM (ZMIERZONY, wersja finalna).** Statek `orbiting` + `idle` wypada między dwie
     ...
     - `MissionSystem._launchTransport:865-869` liczy
       `isRedispatch = isOrbiting || isRemoteDocked`, gdzie `isOrbiting` wymaga
       `position.state === 'orbiting'` **ORAZ `status === 'on_mission'`** ⇒ dla `idle` **false**;
     - gałąź `!isRedispatch` woła `VesselManager.dispatchOnMission`, a ta bramkuje
       (`:401`) `(status idle|refueling) && position.state === 'docked'` ⇒ dla `orbiting` **false**;
     - `false` z dyspozytora → `MissionSystem._abortLaunch:1651`, którego **domyślnym powodem**
       jest `mission.shipUnavailable` ⇒ gracz czyta „Statek niedostępny".

     **Tabela rozstrzygająca** (`OrderService.issueTransport` → `_launchTransport`, wierne
     odwzorowanie bramki `VesselManager:401`; kolumna „z ładownią" to KONTROLA):

     | stan statku | bez ładowni (`cargoMax 0`) | z ładownią (`cargoMax 50`) |
     |---|---|---|
     | `docked` + `idle` | misja (`dispatchOnMission`) | misja |
     | **`orbiting` + `idle`** | **ODMOWA `mission.shipUnavailable`** | **ODMOWA — identycznie** |
     | `orbiting` + `on_mission` | misja (`redispatchFromOrbit`) | misja |

     ⚠ **`cargoMax` NIE MA Z TYM NIC WSPÓLNEGO — obie kolumny są identyczne.** Hipoteza robocza
     z live-gate'u brzmiała „`_launchTransport` traktuje transport jak przewóz ładunku i odrzuca
     statek bez ładowni" (`Fregata I`: `engine_warp`/`warp_tank`/`titanic_plating`/`weapon_missile`,
     `cargoMax 0`). Pomiar jej **nie potwierdza**: `_launchTransport` nie zawiera ANI JEDNEGO
     odwołania do `cargoMax`/`cargoCapacity`, a jedyny predykat ładowni w repo
     (`Vessel.canHaulCargo:487`) ma **zero konsumentów produkcyjnych**. Statek z ładownią, który
     „działał", był **ZADOKOWANY** — czyli w jedynym stanie, który przechodzi. **Ładownia była
     skorelowana, nie sprawcza.**
     ⚠ **Kontrast zmierzony przez właściciela na żywo — ten sam statek, ten sam cel: PPM
     move-to-point DZIAŁA, Transport ODMAWIA — potwierdza mechanizm i jest jego najczystszym
     podpisem:** `moveToPoint` idzie przez `MovementOrderSystem` i **w ogóle nie dotyka**
     `dispatchOnMission`. Dwie drogi do tego samego ciała, jedna zna stan `orbiting`, druga nie.
     ⚠ **Dlaczego uderza akurat w panel mapy:** klikalna populacja mapy to `orbiting`/`in_transit`
     (`ThreeRenderer:4783`/`:1185` — zadokowany nie ma sprite'a), a `orbiting` + `idle` to
     NORMALNY stan spoczynkowy statku po zakończonej misji albo po `undock`. Panel mapy niczego
     nie psuje — **eksponuje** stan, w którym gra już wcześniej odmawiała.
     ⚠ **Poprzednie dwie hipotezy (wybór Outlinerem/Tabem, `_abortLaunch` jako przyczyna)
     WYCOFANE** — pomiar `cargoMax`/stanu je zastępuje. `_abortLaunch` zostaje w opisie, ale jako
     **kanał komunikatu**, nie przyczyna.
     ⚠ **GRANICA DOWODU:** tabela to headless z wiernym odwzorowaniem `VesselManager:401`, nie
     przebieg w przeglądarce. Odtwarza JEDNAK i zgłoszoną odmowę, i zgłoszony kontrast, a kontrola
     ładowni jest rozstrzygająca WEWNĄTRZ fixture'u. Do potwierdzenia na żywo w gate'cie 259.

     **KIERUNEK PODPISANY (właściciel, 2026-09-08) — intencja projektowa jest nadrzędna wobec
     dzisiejszej semantyki:** `Transport` ma być **najprostszym rozkazem „wyślij dowolny statek
     do celu"** i **musi działać dla KAŻDEGO statku**. Dzisiejsze zachowanie to rozjazd INTENCJI:
     silnik traktuje `transport` jak „przewóz ładunku z bazy", gracz jak „leć tam".
     - **OPCJA A (LEAN)** — transport statku, który nie ma czego wieźć (albo nie może wystartować
       ścieżką bazową), **degraduje się do move-to-point**: leci do celu, nic nie wiezie.
     - **OPCJA B** — uczciwy komunikat („brak ładowni" / „statek nie jest w bazie") + osobny
       move-to-point.
     **Do zmierzenia PRZED podpisem slice'u** (właściciel podpisuje z liczb, nie z opisu):
     koszt i promień rażenia A — dotyka `_launchTransport` i/lub predykatu `isRedispatch`, czyli
     ścieżki WSPÓLNEJ dla Rejestru, Dowództwa i panelu mapy, a `dispatchOnMission` ma konsumentów
     poza transportem (każdy typ misji); czy A ma być zawężona do `orbiting` + `idle` (domknięcie
     dziury), czy szeroka (każda odmowa dyspozytora → move-to-point); co się dzieje z ładunkiem
     zadeklarowanym w modalu, gdy lot degraduje się do move-to-point.
     ⚠ **Nie mylić z Findingiem 255**: tam problem to goły punkt OMIJAJĄCY bramkę układu; tu
     ścieżka celowana, która NIE STARTUJE. Ale A wprowadza kolejnego producenta `moveToPoint`,
     więc **255 trzeba przeczytać przed podpisem A**.

     **(dodatkowo, zachowane) Kolizja i18n — PL nie rozróżnia dwóch RÓŻNYCH odmów.**
     `mission.shipUnavailable` (`pl.js:1765`) i `expedition.vesselUnavailable` (`pl.js:3185`) mają
     **identyczny** tekst „Statek niedostępny"; w EN to „Ship unavailable" vs „Vessel unavailable".
     Drugi klucz emituje `FleetManagerOverlay._executeMission:3005`, gdy
     `getVessel(_selectedVesselId)` zwróci `null` — **inna przyczyna, inna warstwa, ten sam
     komunikat dla gracza PL**. ⚠ Rozróżnienie kosztowało rundę diagnozy i udało się **tylko
     dlatego, że właściciel gra po angielsku**. ⚠ Pod **opcją A** pytanie o komunikat częściowo
     się rozpuszcza (nie ma odmowy do zakomunikowania), ale **kolizja kluczy zostaje** i dotyczy
     wszystkich pozostałych ścieżek — nie kasować tego akapitu razem z 259.

     **(kontekst) Macierz akcji panelu — ZMIERZONA**, przydatna każdemu, kto rusza katalog akcji:
     ...
     | `in_transit` | jest → klikalny | **żadnych** (kubełek `in_transit` jest pusty) |

     ⚠ Populacja PANELU jest szersza niż populacja KLIKALNA: `_selectedVesselId` ustawiają też
     `Outliner:668` i Tab (`GameScene:4702`), bez wymogu sprite'a — tak zadokowany statek trafia
     na tę powierzchnię.
     ⚠ **Pozostałe pięć bramek `status !== 'idle'`** (`:561` `_launch`, `:656` `_launchColony`,
     `:762` `_launchFoundOutpost`, `:1021` `_launchPassenger`, `:1461` `_launchReconTarget`) jest
...
| **244** | 🟠 **`cargoUsed` jest DENORMALIZOWANYM agregatem bez rekoncyliacji — po wielu kursach zostaje resztka float przy PUSTEJ mapie `cargo`.** `Vessel.js:636/796` utrzymują `cargoUsed` przyrostowo (`+ actual*weight` / `− actual*weight`; `Math.max(0, …)` tnie tylko ujemne), a `vessel.cargo` jest czyszczone (`delete` przy ≤ 0) ⇒ oba pola się rozjeżdżają. ZMIERZONE (`probe-244-cargo-epsilon.mjs`): cykl SYMETRYCZNY daje **dokładnie 0** dla wszystkich rud (`a+x−x===a`) — pierwsza hipoteza obalona; resztkę produkuje dopiero ASYMETRIA WIELOKURSOWA (kształt `_loadByRarity`: przebieg popytu i przebieg rzadkości ładują TEN SAM surowiec, a `_deliverAndDock` rozładowuje go JEDNYM wywołaniem): 8 kursów → **7,105e-15**, a pełny przebieg 60 gy → **1,4210854715202004e-14, czyli wartość z ŻYWEJ GRY co do bitu**. Resztka przewraca **dokładnie dwie** bramki, obie kształtu `cargoUsed > 0`: `EmpireLogisticsSystem:441` (kurier rusza w drogę powrotną z PUSTĄ ładownią, gdy `_loadByRarity` nic nie wziął, a placówka ma jeszcze cokolwiek ⇒ **jałowy kurs**) i `TransportOrderSystem:246` (zlecenie GRACZA uznaje statek za załadowany). | 🟠 **OTWARTY — poprawka w slice'ie F1+F2 jako OSOBNY commit** (`NT_LINK_PLAN.md`, C3). ⚠ **POŁOWA PIERWOTNEJ HIPOTEZY OBALONA POMIAREM: resztka NIE ZAMRAŻA kuriera.** `in_transit` + `dockedAt=null` + resztka to **normalny stan statku W LOCIE** — ta sama poza i **ta sama wartość** wystąpiły na koronnie ZDROWEJ trasie (`delivered 52`, stolica Nt 676 @ gy 60, `spozniony:false`, `zostalo:+1,64`). Cztery kadłuby „home" z żywego paste'u są więc kandydatami na „w locie", nie na „zamrożone"; rozstrzyga odczyt ZEGARA MISJI (L5), bo okno 3 gy przy kadencji ~9 gy/kurs ma wartość oczekiwaną ~1,3 dyspozycji i **zero obserwacji nie jest dowodem**. ✅ **ZAMKNIĘTY — C3 (`673b11b`)**: NEW `recalcCargoUsed(vessel)` liczy zajętość **Z `vessel.cargo`** (jedno źródło prawdy), wołane na końcu `loadCargo`/`unloadCargo`; **BEZ flagi** (D-Nt-4 — flaga na rekoncyliacji arytmetyki instytucjonalizowałaby rozjazd dwóch pól, które mają być jednym). Sonda po naprawie: **0/10** ścieżek asymetrycznych zostawia resztkę (było 1/10, w tym 8-kursowa 7,105e-15). Keeper `cargo_used_reconcile_smoke` **15/15**, fail-first 12/3 — pinuje inwariant `cargoUsed === Σ qty·weight` po KAŻDEJ operacji oraz OBA skutki (koniec jałowego kursu przez `_advanceRouteCourier`; `TransportOrderSystem:246` nie czyta pustego statku jako załadowanego). |
| **245** | ⚪ **PYTANIE PO SLICE'IE (bliźniak 218, ale na poziomie UNII): `Nt` nie ma jak wejść do popytu stolicy Z KONSTRUKCJI — więc kurier nigdy nie dostaje go w przebiegu popytowym, nawet pod presją.** `EmpireLogisticsSystem._capitalOreNeed` (D-Fe-1 = UNIA) czerpie z DWÓCH źródeł i oba są dla Nt ślepe: **(a) koszty zleceń okrętowych** — ZWERYFIKOWANE grepem, w `HullsData` / `ShipTemplateData` / `ShipModulesData` **nie ma ANI JEDNEGO wystąpienia `Nt`** (kadłuby to Fe/Ti/Cu/Hv, np. eskorta Fe 100 · Ti 130 · Cu 25 · Hv 38), więc to źródło nie może wnieść Nt **nigdy**; **(b) stalle fabryki** — Nt wchodzi wyłącznie recepturami `quantum_cores {Si 6, **Nt 4**, Hv 4, Xe 3, Ti 2, Li 2}` i `antimatter_cells {**Nt 4**, Xe 4, Hv 3, Li 2}`, a te nie ALOKUJĄ się bez popytu z księgi/zapasu (rodzina **217**/**221**). ⇒ unia zostaje **Nt-pusta**, co potwierdza żywy odczyt `popytNaRudy {}` (L4, `GATE-S4-fresh-gy60`) przy jednoczesnym `warp_cores 0 → 1`. Skutek: Nt jedzie wyłącznie przebiegiem rzadkościowym, **za Xe** (ten sam tier 5, ale `Object.keys` stawia Xe pierwsze) i z capem `cargoMax/3` ⇒ **13 sztuk na kurs** przy 16 Nt na jeden `warp_core`. | ⚪ **OTWARTY — OBSERWACJA, nie zlecenie.** Świadomie **NIE rozszerzamy** o to slice'u F1+F2 (decyzja właściciela 2026-09-03): to pytanie o KANAŁ POPYTU, nie o transport, i należy do rodziny 217/218/221 — a tam każda zmiana rusza balans alokacji fabryki AI. ⚠ Pytanie do rozstrzygnięcia PO slice'ie: czy popyt na rudę ma być wyprowadzany z RECEPTUR ŁAŃCUCHA (a nie z kosztów kadłuba i stalli), skoro najdroższe ogniwo warp nie pojawia się w żadnym z dwóch dzisiejszych źródeł. |
...
| **246** | 🟠 **Bramka wejscia do lancucha i lista archetypu NIE ZGADZAJA SIE co do tego, czyj to lancuch.** `ColonyAutoExpander._syncTier3SafetyDemand` (`d44af5e`) mierzy zamoznosc rudami **Fe/Si/Cu/C**, rozlacznymi z wsadem receptur lancucha warp (`quantum_cores {Si, Nt, Hv, Xe, Ti, Li}`, `antimatter_cells {Nt, Xe, Hv, Li}`, `warp_cores {QC 2, AC 2, Ti 8}` — wspolne tylko `Si`). ⚠ **Rozlacznosc jest SWIADOMA** — tresc `d44af5e` argumentuje, ze bramkowanie na ILOSCI wsadu receptury byloby cykliczne, i **ten argument jest poprawny dla predykatu, ktory odrzucil**. Defektem jest SKUTEK: `EmpireArchetypeExpansionist`, ktoremu `antimatter_cells`/`warp_cores` dodano SWIADOMIE (`:47-54` — ma `fusion_power` w kolejce badan), a ktorego opis brzmi „parta ku gwiazdom, dazy do warpu”, **nie przechodzi bramki nigdy**. ZMIERZONE w gy 100, obie galaktyki: pada na **Fe i Cu** (5 607/16 124 oraz 4 931/3 116 wobec 20 000), wiec `getSafetyStockTarget('warp_cores')` czyta **1** przy **286-1218 Nt** w magazynie, a produkcja to **0 QC / 0 AC / 0 WC**. Bootstrap ustawia bonus 49 w gy 0, `_syncTier3SafetyDemand` zeruje go do **gy 0,1** (23 121 wywolan w 100 gy). | 🟠 **ZAMKNIĘTY — E3H WDROŻONY (`d284765`), LIVE-GATE PASS 2026-09-03** (`CHAIN_ENTRY_GATE_CHECKLIST.md` §Wynik; fixture `GATE-S4-fresh-gy60`, gy 60→75). Zatrzask z pasmem (OPEN 0,20 / CLOSE 0,12) + cel proporcjonalny, za flagą `aiTier3ScaledEntry` (ON; OFF = `d44af5e` co do bitu). ZMIERZONE NA ŻYWO: ekspansjonista `celWC` **1 → 12**, zatrzask otwarty ~gy 62, **`QC/AC/WC` 0 → 3/1/7** z własnego `Nt` — archetyp z warpem w kolejce badań wszedł do własnego łańcucha; industrialista `celWC` śledzi `frac` **36→32→26→21→19→14** (proporcjonalnie, nie binarnie); `latch undefined` po wczytaniu mimo `frac 0,702` (fail-closed, D-E3-2, bez migracji); cele gracza bez zmian po obu stronach flagi. ⚠ **Ramię ZAMYKAJĄCE też odpaliło na żywo**: dren `Fe` (**220**) zabrał `frac` 0,286 → 0, zatrzask true → false w gy 66, `celWC` → 1 — ochrona ubogiego imperium z `d44af5e` działa w nowym kształcie. Keeper `ai_tier3_scaled_entry_smoke` **25/25** (fail-first 16/24). Poprzednia treść wpisu (pomiar podpisany, warianty E0-E4) → `CHAIN_ENTRY_PLAN.md`. ⚠ **D-CE-1 nadaje mu znaczenie**: po decyzji „rezerwa gotowosci” (**247**) wejscie do lancucha **= zdolnosc ODPOWIEDZI na nacisk gracza eskorta warp**; imperium za zamknieta bramka nie ma czym odpowiedziec. n=2 galaktyki, zgodnie, mechanizm nazwany. |
| **247** | 🟠 **`warp_cores` maja SUFIT bedacy stala DANYCH, ktorego nic nie drenuje.** `startingSafetyStocks.warp_cores = 50` (oba archetypy) → `setDemandBonus(warp_cores, 49)` ponad baze tier-3 = 1 (`EmpireColonyBootstrap:220-230`, `SAFETY_STOCK_BASE_TIER35`). Po osiagnieciu 50 planista reaktywny nie ma zrodla `safety` i alokacje lancucha spadaja do **zera** (ZMIERZONE: emp_001 gy 100 pod F3S/F3 ma QC 50 / AC 50 / WC 50 i **pusta** liste alokacji). **INWENTARZ KONSUMENTOW (wyczerpujacy grep po calym `src/`):** budowa — `engine_warp` **2** (`ShipModulesData:80`), `engine_warp_mk2` **2** (`:102`), `warp_beacon` **5** (`BuildingsData:1022`), `jump_gate` **20** (`:1049`); biezaca — paliwo skoku `warpFuelPerLY 0,125` rdzenia/LY ciagniete przez `VesselManager._tickRefueling → _refuelTank(..., 'warp_cores', ...)` (`REFUEL_RATES.warp_cores = 2,0`/rok) + przycisk `manualRefuel` gracza. **Zadnego innego konsumenta w `src/` NIE MA** (i18n / `ResourceIcons` / `ShipPickerModal` / `VesselModelResolver` / `StationDepot` / `TRADEABLE_GOODS` / `SaveMigration` v82 = odczyt albo transfer, nie konsumpcja). ⚠ **ZMIERZONE: biezacy konsument ciagnie ZERO w rekach AI** — `warp_cores zuzyte na tankowanie = 0,0` w R0 **i** F3, obie galaktyki, 100 gy; szesc kurierow AI z lacznym bakiem 30 skonczylo z **0,0** w bakach i nie pobralo ani jednego rdzenia (skoki AI biegna na clampie odpornosci paliwowej). Szablony z `engine_warp` (`frigate_laser_escort`, `frigate_missile_escort`, `frigate_system_defender`, `transport_assault` — ten z opcjonalnym DRUGIM silnikiem, do 4 rdzeni) zamawia **wylacznie** `DirectorProduction` pod naciskiem `armedPlayerVesselsInBorderZone`. ⇒ **powyzej ~900-1600 Nt/100 gy dodatkowe neutronium jest bezczynne u imperium, ktore umie go uzyc, i bezczynne w KAZDEJ ilosci u tego, ktore nie umie (246).** | ⚪ **OTWARTY Z PROJEKTU — decyzja właściciela 2026-09-03: STATUS QUO.** ⚠ **AKTUALIZACJA PO GATE'CIE 246 (live, gy 60→75): łańcuch udowodniony OD KOŃCA DO KOŃCA.** emp_001 zbudował **SIEDEM uzbrojonych okrętów z własnej ekonomii** (`director:shipCompleted 3`, `shipQueued 11`, `pressureIncident 6`) — **pierwsze okręty wojenne AI z własnej produkcji w projekcie**. `kadlubyZeSkokiem` **nadal 0 i to jest POPRAWNE**: jedyna eskorta z `warp_tank` (`frigate_laser_escort`) stoi w kolejce (TTL 76,7 — brak DRUGIEGO `warp_core`), a dwa ukończone okręty to wzorzec **251** (`engine_warp` bez baku, `warpFuel.max = 0`). **Jeden cykl fabryki od przewrócenia licznika.** Sufit 50 zostaje sensem rezerwy gotowości — 247 NIE zamyka się razem z 246. Lancuch warp AI to **rezerwa gotowosci** (gromadzi do sufitu, konwertuje pod naciskiem gracza). Obie galezie „tak” (rywalizacja AI-vs-AI; doktryna patrolowa + zdjecie odpornosci paliwowej warpu) → **backlog strategiczny, JEDEN nazwany duzy slot obok GROUND**, z tym inwentarzem jako materialem wyjsciowym. |
| **248** | ⚪ **`_colonyCanSustainRecipe` NIE JEST stabilnym predykatem WEJSCIA.** Dla swojej wlasciwej roli (przycinanie alokacji, ktore nigdy sie nie dokoncza — `FactorySystem:955/959/1300/1408/1521`) jest poprawny, ale dla surowcow bez producenta per-rok czyta **chwilowy ZAPAS** (`stock > 0 || _inventoryPerYear > 0`, `:2040-2060`). Ruda dowozona kurierem producenta nie ma, wiec predykat przewraca sie na pusty magazyn i wraca przy najblizszym rozladunku. ZMIERZONE w tym samym gy: `canSustain('warp_cores')` = **true** dla emp_001-A (`Nt = 1`) i **false** dla emp_001-B (`Nt = 0`) — na imperium, ktore wyprodukowalo 10 rdzeni. **O wartosci predykatu decydowala JEDNA sztuka neutronium.** | ⚪ **OTWARTY — wiazacy dla wariantu E2.** Do pomiaru wchodzi **wylacznie forma z histereza** („utrzymane przez N kolejnych civYears”), a **licznik przejsc otwarta→zamknieta jest jego metryka rozstrzygajaca** (`CHAIN_ENTRY_PLAN.md` §3.2/§5.1). |
| **249** | 🟠 **F5 (abstrakcyjny przeplyw placowka→stolica) ma TRZY prerekwizyty, nie jeden — plus system-slepa metryke odleglosci.** (1) **243** — placowki nie maja portu i nie zdobeda go nigdy (`ColonyAutoExpander._managedColonies` odsiewa `isOutpost`); (2) **pojemnosc handlowa placowki** — `_allocateTC` daje placowce wylacznie bonus z budynkow handlowych, ktorych placowka nie ma ⇒ **TC = 0**; (3) **ekonomia kredytowa AI (`PHASE5_TODO`)** — `CivilianTradeSystem._executeTransfer` obciaza **odbiorce** 2× wartoscia Kr za transport z placowki i **COFA transfer**, gdy kolonia nie ma czym zaplacic; kredyty stolicy AI czytaja **0**. ZMIERZONE z przyznanymi (1) i (2): **1133 Nt probowane w 14 transferach, PRZENIESIONE 0,0, cofnietych 14/14**; po wszystkich towarach 19 590 probowane / 2 260 przeniesione. ⚠ Osobno: `_getDistance` uzywa `DistanceUtils.orbitalAU` (roznica polosi wielkich) — **slepej na uklad**: para cross-system (placowka `sys_024` → stolica `sys_061`) **zawiazala polaczenie** i przeszla bramke zasiegu 10 AU. ⚠ **LEKCJA POMIAROWA:** pierwsza wersja instrumentacji liczyla WYWOLANIA `_executeTransfer` i raportowalaby „1133 Nt przeniesione” — mierz SKUTEK (delta magazynu), nie wywolanie. | 🟠 **OTWARTY.** Rodzina system-slepych metryk: **138** / **142** / **151** / **152** / **153**. |
...
| **263** | 🟠 **`FleetSystem._maybeAutoDockOnReturn` TELEPORTUJE bezwarunkowo, a próg, który wg komentarza ma tego pilnować, NIE ISTNIEJE JAKO ODCZYT.** `:645-668` przepisuje `position.x/y` na współrzędne ciała, `position.state='orbiting'`, **`dockedAt = planetId`** i **`colonyId = planetId`** (re-homing CAŁEJ bazy statku), zeruje misję, ustawia `idle` i emituje `vessel:arrived` — **bez ANI JEDNEGO testu odległości ani układu**. Komentarz nad nim (`FMO:4820`) obiecuje co innego: *„FleetSystem listener … dock'uje gdy vessel dotrze w dystansie `RETURN_DOCK_THRESHOLD_AU` od planety"*. Stała **`RETURN_DOCK_THRESHOLD_AU = 0.5` jest zadeklarowana w `FleetSystem:30` i NIE MA ANI JEDNEGO ODCZYTU** w całym `src/` (grep). ⇒ obietnica komentarza jest martwa od napisania. W parze z **154**/**255** (marker `_pendingReturnDock` stawiają `FMO:4824` i `FleetCommandPanel:393` z celu wybranego BEZ terminu układu) daje to zmierzony stan „`systemId = sys_061` przy `dockedAt` = ciało z `sys_home`" — i ten stan **idzie do zapisu**. ⚠ **D-FDf zdjął ten marker ze ścieżki odwrotu doktrynalnego DOKŁADNIE z tego powodu** (`FleetSystem:584`), ale ścieżka „Powrót do bazy" go zachowała. | 🟠 **OTWARTY — filed w D-147d, świadomie NIE naprawiony w slice'ie 147** (Shape A dotyka wyłącznie admisji rozkazu, nie doku). Naturalny partner slice'u **255+154** (Shape C, producent-side): te same dwa call-site'y stawiają marker. ⚠ Do rozstrzygnięcia przy podpisie: czy `RETURN_DOCK_THRESHOLD_AU` **ożywić** (dok tylko w promieniu), czy **skasować** wraz z kłamiącym komentarzem — dziś stała jest trzecim rodzajem długu: obietnicą bez egzekucji. ✅ **ZAMKNIĘTY 2026-09-09 — `9e1e7d8`, live-gate PASS 6/6.** Rozstrzygnięcie: **SKASOWAĆ, i to jest podparte pomiarem, nie gustem.** `git log -S` pokazał, że próg usunięto ŚWIADOMIE w `7ea94e8` (2026-05-20) jako naprawę Bug F2 („dotarli ale dalej nie dokuja"), z podpisem „snap to teleport — akceptowalny convenience UX w 4X" ⇒ ożywienie stałej **cofałoby zamkniętą naprawę**. Pomiar potwierdza tamtą decyzję, i to WEWNĄTRZ jednego układu (czyli już po naprawie 154): statyczny `targetPoint` z chwili wydania rozkazu nie zna ruchu planety, więc po locie planeta jest **3,15-7,62 AU** od tego punktu (cel na orbicie 9 AU) ⇒ próg 0,5 AU **padałby ZAWSZE**. Zamiast progu odległości konsument dostał **TERMIN UKŁADU** (`isSameSystem`, fail-OPEN — encja bez stempla `systemId` to stary zapis i ma dokować jak dotąd). ⚠ **Sam swap selektora (154) NIE WYSTARCZYŁ** i to była decydująca miara: marker PRZEŻYWA porzucenie rozkazu, więc ZMIERZONE w JEDNYM układzie i BEZ „Stop" — Powrót → nowy rozkaz na 12,00 AU → statek kończy na **1,00 AU zadokowany przy bazie**; rozkaz gracza po cichu przekierowany. Stąd druga połowa naprawy: **producenci stawiają marker PO `issueFleetOrder` i tylko dla `res.accepted`** (to zabiło DRUGIE, utajone źródło: statek z ODRZUCONYM rozkazem zostawał z markerem), a sprzątanie stoi na **wejściu `issueOrder`** (przed `_preemptSnapshot`) i w **`_releaseOrder` dla każdego przejścia terminalnego poza `completed`** — cztery z pięciu ścieżek, wliczając unieważnienie PRZY WCZYTANIU (`MOS:136`), czyli dokładnie „lepki marker prosto do nowej sesji". ⚠ Sprzątanie w `_preemptCommit` **sprawdzone i ODRZUCONE**: wycierałoby marker rozkazu, który sam właśnie instalujemy (producent stawia go przed `issueOrder`, więc `_preemptSnapshot` widzi już NOWY) — złapane przed uruchomieniem czegokolwiek. ✅ **§0 gate'u odpowiedział POMIAREM na pytanie o leczenie starych zapisów: pre-skan kampanii właściciela na chimery (`dockedAt` w innym układzie niż statek) zwrócił PUSTĄ listę — ZERO pacjentów, więc przebieg leczący jest zbędny.** (Same markery i tak są runtime-only — nie ma ich w `VesselManager.serialize`.) Keeper `return_dock_family_smoke` T3/T4/T5/T5b/T5c/T6/T7 (T6 z kontrolą na zmutowanej kopii — pilnuje, żeby nikt nie „naprawił" tego przez powrót progu). |
...
| **266** | 🟠 **„Docked" jest DOMYŚLNĄ ETYKIETĄ w DWÓCH niezależnych miejscach — panel raportuje FALLBACK jako FAKT, i żaden z tych miejsc nie czyta `dockedAt`.** ZMIERZONE WYKONANIEM (`buildRosterRows` + realna mapa widoku): stan `undefined` → **`statusKey = 'docked'`** (`FleetGroupPanelLogic:83`, `?? 'docked'`); stan NIEZNANY (np. `'exploring'`) → klucz nie trafia w mapę i widok robi `?? 'fleetGroup.statusDocked'` (`FleetGroupPanel:345`, `FleetCommandPanel:297`) → **„Docked"**; brak całego `position` → to samo. Trzy różne stany świata renderują się jako „zadokowany", a **`position.dockedAt` nie jest w tej ścieżce czytany ani razu**. ⚠ **TRIGGER: obserwacja z live-gate'u 147** — rejestr floty pokazywał `v_13` jako „docked at Mstow", gdy stan statku był `systemId: sys_020`, `dockedAt: null`, `_pendingReturnDock: null`, `_pendingDock: null`. ⚠ **GRANICA DOWODU, NAZWANA:** pinpoint KTÓREJ powierzchni dotyczył tamten odczyt wymaga `position.state` **z chwili renderu**, a zrzut go nie zawierał. Drugi kandydat na tamto konkretne zdanie to `FleetManagerOverlay._getLocationText:3819-3830` / wiersz `:3717-3722` — te keyują na `position.state` i nazwę biorą z `_resolveName(position.dockedAt)`, a `_resolveName(null)` zwraca **`'???'`** (`:171`), więc dla `dockedAt: null` napisałyby „Hangar: ???", nie „Hangar: Mstow" ⇒ albo `dockedAt` **nie był** null w chwili renderu (zrzut i render nie były jednoczesne), albo powierzchnią był jeden z panelo­wych fallbacków wyżej. **Defekt fallbacku jest jednak reprodukowalny NIEZALEŻNIE od tego rozstrzygnięcia.** ⚠ **SPROSTOWANIE PRZESŁANKI (audyt 2026-09-15, ZMIERZONE):** silnik produkuje DOKŁADNIE trzy tokeny `position.state` ('docked' ×3 pisarzy / 'in_transit' ×16 / 'orbiting' ×29; fabryka `Vessel.js:158`; `SaveMigration` nie pisze stanu), a **każdy pisarz 'docked' ustawia `dockedAt` w tym samym statemencie** ⇒ docked ⇒ dockedAt ≠ null Z KONSTRUKCJI. Stan `undefined`/nieznany jest osiągalny **wyłącznie** z zapisu bez pola (`VesselManager.restore` robi `{ ...vd.position }` → `{}`, bez leczenia — wykonane) albo z przyszłego czwartego tokena. ⇒ **fallback „Docked" był LATENTNY**: nie mógł odpalić na żadnym statku, który zrobił silnik. A skoro tak, to **żadna żywa powierzchnia statusu nie mogła wypisać „Docked" obok „Mstow" dla rekordu z `dockedAt: null`** — leg-D-owy „docked at Mstow" **NIE pochodził z etykiety statusu**. Jedynym napisem tej postaci w OBU słownikach jest `vessel.docked: 'Docked at {0}'` — **linia DZIENNIKA MISJI** pisana przy ostatnim dokowaniu (`VesselManager:725`) i renderowana (ostatnie 8, od najnowszej) w detalu statku w Command (`_drawMissionLog`), a drugim „Mstow" na tym samym detalu jest `_baseText` (kolonia macierzysta, `:8518`). Statek z legu D przyleciał ścieżką `VesselManager:2818` (obrzeże, `orbiting` + `dockedAt=null`) i renderował „Orbiting" / „In orbit" / **„Orbit: ???"** — etykieta nie kłamała na tamtym rekordzie; **historia (albo baza) stała tam, gdzie czyta się status**. ⚠ Jedynym OSIĄGALNYM kłamstwem etykiety był ten właśnie kształt: **'orbiting' PRZECIĄŻONE** (orbita ciała ALBO swobodny dryf z `dockedAt=null` — ≥8 żywych producentów: `MOS:1264` engage, `:1637` lot w pusty punkt, `:2011`, `DSCS:1370`, `VCS:387`, `VM:2556`, `VM:2818`, `ReturnJump:62`), które panele nazywały „Orbiting" niczego, a Command „Orbit: ???" / „⊙ ???". ⚠ Rejestr liczył dwa site'y — było ich **sześć** w rodzinie „domyślnie Docked" (`?? 'docked'` ×3: `FleetGroupPanelLogic:83`, `FleetPictureLogic:239`, `Outliner:369`; `?? 'fleetGroup.statusDocked'` ×2: `FleetGroupPanel:345`, `FleetCommandPanel:300`; `?? byState.docked` ×1: `Outliner:370`) plus **cztery łańcuchy zgadujące INNE słowo** dla tego samego nieznanego (Command „In flight" ×5, K3 „Idle" `FleetRegistryLogic:94`, tooltip 3D „Bezczynny" `ThreeRenderer:3495`, NavPeek — cicha luka w sumie). ⚠ `StationPanel:255` to kuzyn tylko z kształtu — producent `gatherStationTraders` emituje zbiór zamknięty, fallback nieosiągalny. | ✅ **ZAMKNIĘTY 2026-09-15 — KANON (D-266c).** Decyzje właściciela: **D-266a** fallback = (i) HONEST z DEDYKOWANĄ parą `fleetGroup.statusUnknown` ('Unknown' / 'Nieznany'; bez reużycia `vessel.unknown` — jego dom to „nieznany STATEK") · **D-266b** rozszczepienie dryfu = TAK, druga para `fleetGroup.statusInSpace` ('In space' / 'W kosmosie' — brzmienie za istniejącym `navPeek.fleet.inSpace`) · **D-266c** zakres = KANON: NEW `src/utils/VesselStatus.js` (zero importów, fail-CLOSED; wzór `ColonyOwnership`/`SystemExploration`) — `resolveVesselStatus(vessel) → { token, bodyId }`, zbiór `VESSEL_STATUS_TOKENS` = in_transit · orbiting · in_space · docked · unknown; docked bez `dockedAt` (niemożliwe z konstrukcji) → 'unknown', nie zgadywane. **Konsumenci (8 plików):** `FleetGroupPanelLogic.buildRosterRows` (statusKey = token, dockedAt = bodyId) · `FleetGroupPanel` / `FleetCommandPanel` (prywatne mapy STATUS_KEY USUNIĘTE → `vesselStatusLabelKey`, sufiks ciała gated na `bodyId`) · `FleetPictureLogic.buildShipEntry` (token + aktywność; K3/Dok taktyczny/plakietki) · `FleetRegistryLogic` (STATE_LABEL_KEYS + fallback „Idle" → „Unknown") · `Outliner` (grupy = tokeny kanonu w jego kolejności, własne nagłówki „In space"/„Unknown") · `FleetManagerOverlay` (STATUS_COLORS po tokenie, wiersz listy `◌ In space` / `? Unknown` zamiast „⊙ ???", `_getLocationText`, `_statusText` — dryf dzieli test „utknął z dala od bazy", `_warpStatusLabel` in_space=idle, detal wroga) · `ThreeRenderer` (tooltip) · `NavPeekProviders` (nagłówki „In space n"/„Unknown n" komponowane bez nowego klucza `{0}`). **Trzy polskie literały w tych łańcuchach ZOSTAJĄ** (dopisane do 269). ⚠ Promień rażenia = czyste wyświetlanie, ZMIERZONE: `countActionable`/`summarizeFleetGroup`/`manualRefuel`/`undockToOrbit`/stożek wydechu/`VesselManager` czytają SUROWY `position.state` (pin T6) — token `in_space` NIE jest stanem silnika i nie wolno go zapisać do `position.state`. Zapis **v101 bez migracji**, bez flagi (rollback = revert). **i18n: dokładnie dwie nowe pary** (pl=en=3345), seria „zero nowych kluczy" przerwana świadomie; `fleetPicture.state.idle` stracił jedynego producenta (zostaje w słowniku jako nieużyty, do sprzątania przy okazji). **Keeper `vessel_status_label_smoke` 84/84**, T0-T7 wg szkicu (T0 enumeruje pisarzy `position.state` ze ŹRÓDŁA regexem `\s`-tolerantnym — czwarty token w silniku pali keeper GŁOŚNO; T1 zbiór tokenów brany Z T0, nie z tablicy; T3 leg-D z kontrolą `dockedAt:'h2'`; T5 zgoda nagłówka z rosterem niezależna od wybranego słowa; T6 kontrola „tylko etykieta"; T7 `map_vessel_panel_smoke` w procesie potomnym 120/120). **Fail-first w REALNYM `git worktree` na `9127f14` (Finding 270): 44 PASS / 40 FAIL** — czerwone DOKŁADNIE T2 (17), T3 (9), T4 (12), T5 (1: „2 == 1" — sedno) + 1 kontrola kanonu w T0; T1/T6/T7 zielone (kontrole). Sweep **224/224 0 FAIL** · `check-i18n` PASS (zapadka trzyma, baseline bez zmian). ⚠ Interakcja ze STARYM zapisem-chimerą (`docked` + `dockedAt` z innego układu): etykieta renderuje **to samo co przed naprawą** („Docked ‹ciało›") — poprawnie, bo raportuje REKORD; rozjazd układu to 256/263 (admisja zamknięta; §0 gate'u 256 zwrócił zero pacjentów). ⚠ NIE nadano numeru obserwacji „linia dziennika misji czyta się jak status" (decyzja o numerze = właściciel). ✅ **LIVE-GATE 2026-09-15 PASS** (właściciel, na żywo, klient EN, po twardym odświeżeniu): **§1** zdrowa trójka — ZERO zmiany wizualnej (zrzut rejestru: Docked+ciało, In orbit+„Stacja Mstow", In flight na kursach międzyukładowych); ⚠ **obserwacja, nie defekt (zapisana):** statek W SKOKU WARP (`orbiting`+`dockedAt:null` na czas tranzytu) renderuje „In space" z aktywnością „Warp jump" obok — kanon prawdomównie raportuje POZYCJĘ, kolumna aktywności rozstrzyga (dwie osie, jak w §266 zapisano: status fizyczny ≠ misja). **§2** słowo dryfu — PASS wizualnie na WSZYSTKICH powierzchniach I z konsoli: `v_18` `position {state:'orbiting', dockedAt:null}` → `resolveVesselStatus` `{token:'in_space', bodyId:null}`. **§3** słowo „Unknown" — **NIE ćwiczone na żywo** (populacja pusta z konstrukcji; krok opcjonalny pominięty) — stoi na **T2 headless**, wzór uczciwości 271. **§4** szkoda stanu — `JSON.stringify(v.position)` IDENTYCZNE przed/po otwarciu rejestru, panelu grupy, detalu Command, Outlinera i tooltipa mapy — kanon tylko czyta. PASS. Commity: C1 kanon+i18n · C2 konsumenci+keeper · C3 docs. |
| **267** | 🟠 **`patrolManual` — TRZECI producent punktu z ramki KAMERY w tym samym pliku, poza zasięgiem legu D.** `RightClickMenu:278` (opcja `patrolManual` z `MENU_OPTIONS_BY_TARGET.empty`) uruchamia picker waypointów; te wracają jako gameplay px **z kamery** (`GameScene:5931` → `um.addPickerWaypoint(gp)`) i idą do `buildPatrolFromWaypoints` (`OrderDispatcher:149`) → `mos.issueOrder({type:'patrol', patrolRoute})`. Ścieżka **nie przechodzi przez `buildOrderSpec`**, a `patrol` buduje własny order z pominięciem `_issueMoveToPoint` (`MOS:1667`), więc **żadna bramka układu jej nie ogląda**. ZMIERZONE: statek w `sys_020`, kamera `sys_home`, trasa `[(1 AU,0),(4 AU,0)]` z ramki kamery → `{ok:true, orderId:'mo_3'}` — statek patroluje punkty odmierzone od CUDZEJ gwiazdy. ⚠ Ta sama klasa co 255 (goły punkt bez ramki), ale **inny producent i inny typ rozkazu**, więc osobny numer — dokładnie jak 256 wobec 255. | ⬜ **OTWARTY, świadomie poza legiem D (D-LD5).** Pinowany ODWRÓCONYM pinem `map_click_frame_smoke` **T9b** (pin pinuje DEFEKT; **ma paść**, gdy 267 zostanie domknięty). Naprawa = ten sam termin `_outOfCameraFrame` w gałęzi `patrol` (`RightClickMenu:274-291`), ale ⚠ **wymaga podpisu co do MOMENTU odmowy**: przy WYBORZE opcji (przed pickerem — tańsze, ale gracz traci klik) czy przy FINALIZACJI (po zebraniu waypointów — uczciwsze wobec gracza, który mógł w międzyczasie przełączyć układ, ale wyrzuca zebraną pracę). ⚠ Osiągalność: `patrolManual` jest jedyną pozycją menu wchodzącą w tryb pickera z PPM. ✅ **MOMENT ODMOWY ROZSTRZYGNIĘTY REGUŁĄ D-89c (2026-09-10): FINALIZACJA, nigdy ARM** — bramka ARM-time jest NIEPEŁNA, bo nic nie kasuje pickera na `system:switched` (ZMIERZONE na wierszach 8/9: picker po przełączeniu układu NADAL uzbrojony). Otwarta zostaje wyłącznie **mitygacja UX**: czy odmawiać przy PIERWSZYM waypoincie (gracz traci jeden klik zamiast całej trasy), czy dopiero po ENTER. ⚠ Predykat jest już gotowy i wspólny — `utils/CameraFrame.js` (D-89b); `patrol` NADAL nie wchodzi do `POINT_SOURCED_ORDER_TYPES` (152 / T8e). ⚠ 267 **nie dzieli finalizatora** z wierszami 8/9 (`patrolWaypoints` idzie `addPickerWaypoint` ⇒ ENTER ⇒ `UIManager.finalizePickerMode`), więc naprawa wierszy 8/9 go NIE dotknęła — T9b zostaje ODWRÓCONY. ✅ **WDROŻONE 2026-09-15 — D-267a = (A), podpis właściciela; live-gate PASS (§2/§4/§5; §1/§3 PASS-with-finding → 273; re-run §3 A=3, B=2) — ZAMKNIĘTY 2026-09-15.** Każdy waypoint oceniany **W CHWILI POŁOŻENIA, w ramce SWOJEGO kliku** (per-punktowe odczytanie D-89c: położenie punktu JEST jego finalizacją); punkt spoza ramki statku odrzucany NATYCHMIAST i GŁOŚNO (Dziennik `fleet`/`warn`: `log.el.orderRejected` + `describeOrderFail` = nazwa statku + `vessel.reasonTargetOtherSystem`, **zero nowych kluczy**), a trasa zebrana do tej pory **PRZEŻYWA**; ENTER finalizuje trasę już-w-całości-ważną. ⚠ **STOP-klauzula NIE zaszła — zmierzone:** przy kliku picker MA obie strony pytania: `metadata.vesselId` (stawia producent) i kamerę (`activeSystemId` w tej chwili), a bufor trasy żyje w `UIManager._pickerState.waypoints` — placement-time jest strukturalnie możliwe. **Kształt (trzy warstwy, jeden termin):** producent `RightClickMenu` (JEDYNE wejście `patrolWaypoints` z `vesselId`) wstrzykuje `metadata.validateWaypoint` → CZYSTA maszyna `PickerStateMachine.addWaypoint` woła hook i przy wecie NIE rusza bufora (`{ok:false, reason, vetoed:true}`; maszyna nadal nie zna `window`) → `UIManager.addPickerWaypoint` zamienia weto na `ui:pickerWaypointRejected` → HUD (`GameScene._createPickerHUD`) pokazuje nazwę statku + powód przez `describeOrderFail`. Picker POI (`intent:'create_poi'`) i debug NIE podają weta (POI bez `systemId` — 152); `patrol` NADAL poza `POINT_SOURCED_ORDER_TYPES` (T8e). **Dwie obrony w głąb, obie zmierzone jako osiągalne** (picker nie kasuje się na `system:switched`, a czas płynie podczas klikania): (a) ramka TRASY = kamera przy PIERWSZYM przyjętym punkcie — punkt z innej ramki odrzucany nawet, gdy statek „przyleciał" tam i kamera poszła za nim (bez tego trasa mieszałaby dwie ramki); (b) przy ENTER statek musi NADAL stać w ramce trasy (`systemIdOf` vs ramka) — inaczej odmowa bez rozkazu. Przy okazji trzy dotąd CICHE porażki tego callbacku (`buildPatrolFromWaypoints`, `issueOrder`) idą tą samą głośną ścieżką. ⚠ **T9b (`map_click_frame_smoke`) NIE MÓGŁ PAŚĆ SAM Z SIEBIE** — ta sama klasa co T9a w D-89d: wołał `buildPatrolFromWaypoints` + `mos.issueOrder` WPROST, a MOS jest dla tras bez ramki z definicji (spec `patrolRoute` = gołe punkty, 152). Przecelowany na producenta + prawdziwą maszynę (execution pin); stara asercja została jako **T9b-ctl** (MOS pozostaje przepuszczalny). **Keeper NEW `patrol_waypoint_frame_smoke` 42/42** (T1-T9; T3 = trasa przeżywa: P1 ✓ → zmiana układu → P2 ✗ → powrót → P3 ✓ → ENTER → `patrol` z DOKŁADNIE [P1, P3]; T6 kontrole POI/debug/jedna ramka; T7 piny źródłowe CRLF-safe; T8 i18n w PL i EN bez sluga; T9 kontrola MOS). **Fail-first w REALNYM `git worktree` na `e087507`: 21 PASS / 21 FAIL** — czerwone T1 (1) · T2 (5) · T3 (6) · T4 (2) · T5 (2) · T7 (3) · T8 (2 piny tekstu wpisu); T6/T9 zielone; `map_click_frame` **42/1** (pada dokładnie przecelowany T9b) → po naprawie **43/43**. Sweep **225/225 0 FAIL** · `check-i18n` PASS (pl=en=3345, zapadka bez zmian). Zapis v101 bez migracji, bez flagi (rollback = revert). ⚠ Obserwacja, nie naprawa: baner pickera (`GameScene:5148/5150/5158`) nadal po polsku — Finding 269; nowy tekst odmowy idzie przez `t()`. ✅ **LIVE-GATE 2026-09-15 (właściciel, klient EN):** **§2 PASS** — odmowa przy położeniu: baner + jedna linia Dziennika, `waypoints.length` 0 · **§4 PASS** — picker POI nietknięty · **§5 PASS** — stan bajt w bajt po odrzuconym kliku · **§1/§3 PASS-with-finding** — semantyka ROZKAZU poprawna (rozkaz powstał, status „In flight", trasa), ale sprite 3D nie ruszył, a przy anulowaniu statek „nagle" stanął przy waypoincie 2/3 ⇒ **Finding 273** (render, PRE-EXISTING od M2b, zmierzony bajt w bajt na `e087507` i drzewie 267, niezależny od odległości). Błędy konsoli podczas §1: NIE SPRAWDZONE. ✅ **Celowany re-run §3 (właściciel, semantyka rozkazu, sprite ignorowany):** **A** — jedna ramka, 3 waypointy → `patrolRoute.length` = **3** (oczekiwane 3) · **B** — scenariusz przeżycia (P1 ✓ / klik obcy ✗ / P3 ✓ / ENTER) → `patrolRoute.length` = **2** (oczekiwane 2), dokładnie jedna linia odmowy w Dzienniku. Commity: `fix(267)` + `docs(267)`. |
...
| **273** | 🔴 **Statek pod rozkazem `patrol` (i `escort`) NIE PORUSZA SPRITE'A 3D — symulacja jedzie, render stoi.** ZGŁOSZONE NA LIVE-GATE 267 §1/§3 (właściciel): patrol manualny, 3 waypointy, ENTER → rozkaz powstał, status „In flight" (kanon 266 prawdomówny), ale sprite ani drgnął; po „Anuluj rozkaz" statek „nagle" pojawił się przy waypoincie 2/3. MECHANIZM (źródło + wykonanie): `VesselManager._updatePositions:2303` wrzuca do `moving[]` (→ `vessel:positionUpdate`, jedyne, czym `ThreeRenderer._syncVesselPositions` rusza sprite `in_transit`) statki sterowane rozkazem WYŁĄCZNIE dla `pursue | intercept | engage`; `patrol` i `escort` mutują `position.x/y` wprost w `MOS._tickPatrolOrder` / `_tickEscortOrder`, a `_issuePatrol` ZAWIESZA misję (`mission=null`) i stawia `in_transit` ⇒ statek nie trafia w ŻADNĄ gałąź (`in_transit && m` wymaga misji) ⇒ **`vessel:positionUpdate` nigdy go nie niesie**. ZMIERZONE sondą na OBU drzewach (pristine `e087507` i drzewo 267, BAJT W BAJT): patrol 30 AU → 31,98 AU w 200 tików, indeks waypointu 0 → 1, **0/200** ładunków `positionUpdate` z tym statkiem (kontrola `moveToPoint`: **200/200**); statek przy 2 AU identycznie 0/200 ⇒ **odległość od centrum NIE jest przyczyną** (brak jakiegokolwiek cullingu sprite'ów po dystansie — grep czysty; „daleko od centrum" to miejsce, nie mechanizm). „Nagłe pojawienie się" przy anulowaniu = `cancelOrder` → `_stopVesselMotion` pisze `orbiting` + `dockedAt=null` → następna klatka `ThreeRenderer._tickOrbitingVessels` bierze gałąź wolnego dryfu i czyta REALNE x/y (poprawka desync z M4) — pierwszy kontakt renderera z pozycją, którą symulacja miała od dawna. Wariantu „przypięty do starej orbity" NIE MA: `_issuePatrol` emituje `vessel:launched`, a `OrbitalSpaceSystem` na nim zwalnia orbitę. ⚠ PRE-EXISTING od **M2b C6/C7 (`4929b39`/`28cd5c6`, 2026-04-27)** — lista `isOrderControlled` dostała ostatni kształt w `e91dabc` (M4 P3, `engage`) i nigdy nie objęła patrolu/eskorty; mapa taktyczna czyta x/y wprost, więc TAM ruch był widoczny — to tłumaczy, czemu przeżyło pięć miesięcy. | 🔴 **OTWARTY — filed z gate'u 267, NIE regresja tego slice'u** (diff 267 w `GameScene`/`UIManager` = jeden import + jeden listener banera + jedna gałąź emitu weta; zero kontaktu z pętlą renderu/sync; zmierzone identycznie na obu drzewach). Podejrzany kształt naprawy: dopisać `patrol` i `escort` do `isOrderControlled` w `_updatePositions` (jedna linia; MOS ustawia velocity sam, jak przy pursue) — ale to ZMIANA RENDERU dla wszystkich patroli (POI też) i eskort, więc **własny slice z własnym gate'em**, nie dopisek do 267. Repro headless: sonda `probe_patrol_render` (scratch) — 200 tików, licznik ładunków `vessel:positionUpdate` per statek. |
