...
| **225** | 🔴 **`ColonyAutoExpander` nie skaluje KARMICIELI — cele `farm`/`habitat`/`mine` są STAŁE we wszystkich checkpointach.** `targets/industrialist.js` (Expansionist to klon behawioralny i **dzieli te same targety**, `ColonyAutoExpander:48-56`): **farm 2 → 2 → 2 → 2**, **habitat 1 → 1 → 1 → 1**, **mine 2 → 2 → 2 → 2**, `well` 2 → 2 → 3 → 3, a rośnie tylko `factory` 3 → 4. ⇒ ekspander skaluje **konsumentów**, nie karmicieli. `food/rok` spada wraz ze wzrostem populacji, aż wchodzi głód. ZMIERZONE (`DirectorHarness`, stolica AI): `food/rok` **+17,6 (gy0) → +3,5 (gy5) → −8,9 (gy10, GŁÓD)**, populacja **24 → 19 → 11 → 4**. | ✅ **NAPRAWIONY 2026-09-01** — R4 / `FEATURES.aiScaleBasicInfra`: `farm`/`well` skalowane **populacją**, nie checkpointem (`_feederTarget`: `ceil(pop × POP_CONSUMPTION[res] × 1,25 / rates[res])`, próg arytmetyczny — 16 POP na farmę). ⚠ **DWIE PRZESŁANKI SLICE'U OBALONE POMIAREM:** (1) **energia NIE była statyczna** — ekspander buduje **6 → 13** elektrowni przy celu 5 (moduł survival, `:253-258`), więc skalowanie energii byłoby NO-OPEM wyglądającym na naprawę; (2) **sam wzrost liczby farm NIE wystarcza** — R4 bez bramki **226** dał pop 27 → 16 → **3**, bo `empPenalty` dzieli produkcję WSZYSTKICH budynków, w tym nowych farm. Dominującym członem naprawy okazał się **226**, nie 225. `FE_SUPPLY_PLAN.md` §10 |
| **226** | 🔴 **PĘTLA ŚMIERCI: moduł survival dokłada elektrownie przy ujemnym bilansie, a każda dokłada ETAT, którego nie ma kto obsadzić.** `ColonyAutoExpander:253-258` buduje `solar_farm` przy KAŻDYM `balance < energy_balance_min` (komentarz: *„najwyższy priorytet, brownout psuje wszystko"*). `solar_farm` ma `popCost 0.25` ⇒ `jobs 1`, a produkcja jest mnożona przez `empPenalty = laborer / getSlotDemand('laborer')` (`BuildingSystem._applyTechMultipliers`, gałąź `val > 0`). Gdy obsada spada, nowa elektrownia **produkuje ZERO i podnosi mianownik** — bilans dalej ujemny, więc powstaje kolejna. ZMIERZONE: **solar 11 → 28 → 30 przy `laborer = 0` i `avail = 0,00` przez cały czas** — trzydzieści elektrowni nie dało ANI JEDNEJ jednostki energii. ⚠ Stąd `production: 0` z live-readu R0 (`{"production":0,"consumption":123.15,...}`) — nie brak mocy, tylko brak RĄK. | ✅ **NAPRAWIONY 2026-09-01** — bramka pod `aiScaleBasicInfra`: przy obsadzie `laborer/demand < STAFFING_CRITICAL (0,35)` deficyt jest **OBSADOWY, nie mocowy**, więc gałąź survival NIE buduje mocy; ratunkiem jest żywność. ⚠ **TO JEST DOMINUJĄCY CZŁON NAPRAWY, nie 225** — zmierzone side-by-side na `DirectorHarness` (gy 30): bez bramki pop **3**, `avail 0,00`, Fe **0**, solar 30; z bramką pop **36**, `avail 0,99`, Fe **19 826**, solar **10**. **Mniej elektrowni = więcej energii** — to jest cały dowód pętli. ⚠ Kontrola d44af5e: placówki **5 vs 1** ⇒ ekspansja ROŚNIE, nie jest przesuwana. |
