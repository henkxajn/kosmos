...
fleetpicture 81/81, map_labels 37/37.

---

## 10. KROK B — SLICE'OWANIE FAZY 3 (rejestr K3, `FEATURES.fleetRegistry`)

Wg planu §5 + A.5 + wymagania twarde Filipa.

**Rozstrzygnięcie [MAPA]/[REJESTR]: POD-WIDOK w zakładce SYSTEM TACTICAL** (stan `_tacticalView`),
NIE osobna zakładka. Uzasadnienie (kryterium najmniejszej ingerencji): tab-bar overlaya jest
globalnym dispatcherem 5 zakładek o RÓŻNYCH layoutach — nowa zakładka wymagałaby zdublowania
3-kolumnowego layoutu tactical (lewa lista + prawy panel MAJĄ zostać w obu widokach — wymóg twardy),
podczas gdy pod-widok wymienia WYŁĄCZNIE środek (`_drawCenter` ↔ `_drawRegistry`); zero zmian w
tab-barze, hit-zonach zakładek, lewej liście i prawym panelu.

- **Slice 3a — czysta logika:** `src/ui/FleetRegistryLogic.js` — `buildRegistryRows(vessels, ctx)`
  (wiersze WYŁĄCZNIE z `buildShipEntry`; kolumny Nazwa·Rola·Flota·Układ·Stan·Zadanie·ETA·⛽·Alert
  jako dane+klucze i18n), `sortRows` (każda kolumna, oba kierunki, stabilny tie-break po id),
  `filterRows` (układ dynamicznie + `__transit` × rola × szukajka case-insensitive po nazwie) ·
  `src/ui/TimelineLayout.js` — `yearToX/xToYear` (zakres widoku), `layoutTimelineRows`
  (paski z `buildTimelineRows` przycinane do zakresu, lane per wiersz), `nowLineX`, zoom zakresu.
  Smoke `tmp_fleet_registry_smoke.mjs` (sort/filtry łączone/layout osi/spójność tabela↔oś).
- **Slice 3b — przełącznik + tabela:** `FEATURES.fleetRegistry` + przycisk `[MAPA] [REJESTR]` w
  nagłówku centrum (flaga OFF → przycisk niewidoczny, `_tacticalView` zawsze 'map', zero kosztów);
  `_drawRegistry`: nagłówki z sortem (klik, strzałka kierunku), chipy filtrów (układy + 🌀 tranzyt,
  role), wiersze ~24 px + scroll/clip/culling wzorem `_drawLeft`, klik wiersza = `setSelectedVesselId`
  (wspólna selekcja), 🎯 = zamknij overlay + (inny układ → `switchActiveSystem`) + `vessel:focus`,
  checkboxy = reuse `_multiSelectedIds` + istniejący pasek „Przypisz (N)"; wyszukiwarka = DOM input
  adaptacją `TradeOverlay._openQtyInput` (żyje tylko w widoku REJESTR). i18n PL+EN.
- **Slice 3c — oś czasu:** `_drawTimeline` pas pod tabelą (~1/3 wys., zwijany ▾): wiersze =
...
- **Slice 3d — spięcia + domknięcie:** chip 🌀 tranzyt → REJESTR z prefiltrem transit (zamiast
  ...
  ≤2 ms/klatkę przy 100 statkach, test save sprzed arca (v90 bez migracji). **Live-gate CC (Chrome/Power Test):** Y → top-down (phi
