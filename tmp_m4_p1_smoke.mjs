...
// NOTE: CURRENT_VERSION ewoluuje per faza. T2.1 sprawdza tylko że jest >= 70
// (M4 P2 minimum), żeby tested nie wymagał update'u przy każdym kolejnym bumpie.
test('T2.1 CURRENT_VERSION >= 70 (M4 P2 minimum)', () => {
...
test('T2.2 migrate v68 save → v70 + driftIdle/lowFuelDrift null defaults', () => {
  ...
  assertEq(result.version, CURRENT_VERSION, 'version bumped');
...
test('T2.5 migrate v66 save → łańcuch dochodzi do CURRENT_VERSION', () => {
  const data = { version: 66, civ4x: {} };
  const result = migrate(data);
  assertEq(result.version, CURRENT_VERSION, 'chained migration');
});
test('T2.6 migrate save z przyszłości (CURRENT+1) → error', () => {
